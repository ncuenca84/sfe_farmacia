<x-emisor-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">Reporte de Ventas Detallada</h2>
    </x-slot>

    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form method="GET" action="{{ route('emisor.reportes.ventas-detallada') }}">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('emisor.reportes.export-ventas-detallada', request()->query()) }}" class="bg-green-600 text-white px-3 py-2 rounded hover:bg-green-700 text-sm flex items-center" title="Exportar Excel">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mb-4">
        <div class="px-4 py-3 bg-gray-50 border-b flex flex-wrap justify-between items-center gap-2">
            <div class="flex items-center gap-4 text-sm">
                ...
                <span class="text-gray-500">{{ $factura->fecha_emision->format('d/m/Y') }}</span>
                <span class="text-gray-700">{{ $factura->cliente->razon_social ?? 'N/A' }}</span>
                <span class="text-gray-400">{{ $factura->cliente->identificacion ?? '' }}</span>
            </div>
            <div class="text-sm">
                <span class="text-gray-500">Subtotal: ${{ number_format($factura->total_sin_impuestos ?? 0, 2) }}</span>
                <span class="text-gray-500 ml-3">IVA: ${{ number_format($factura->total_iva ?? 0, 2) }}</span>
                <span class="font-bold text-gray-900 ml-3">Total: ${{ number_format($factura->importe_total ?? 0, 2) }}</span>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2 text-sm text-gray-500 font-mono">{{ $detalle->codigo_principal ?? '-' }}</td>
                        <td class="px-4 py-2 text-sm text-gray-900">{{ $detalle->descripcion }}</td>
                        <td class="px-4 py-2 text-sm text-gray-900 text-right font-mono">{{ number_format($detalle->cantidad, 2) }}</td>
                        <td class="px-4 py-2 text-sm text-gray-900 text-right font-mono">${{ number_format($detalle->precio_unitario, 4) }}</td>
                        <td class="px-4 py-2 text-sm text-gray-900 text-right font-mono">${{ number_format($detalle->descuento ?? 0, 2) }}</td>
                        <td class="px-4 py-2 text-sm text-gray-900 text-right font-mono">${{ number_format($detalle->precio_total_sin_impuesto ?? 0, 2) }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
