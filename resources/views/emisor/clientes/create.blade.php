<x-emisor-layout>
    ...
    <div class="bg-white rounded-lg shadow p-6" x-data="consultaCliente()">
        <form method="POST" action="{{ route('emisor.configuracion.clientes.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="tipo_identificacion" x-model="tipoIdentificacion" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                        <option value="04" {{ old('tipo_identificacion') == '04' ? 'selected' : '' }}>04 - RUC</option>
                        <option value="05" {{ old('tipo_identificacion') == '05' ? 'selected' : '' }}>05 - Cedula</option>
                        <option value="06" {{ old('tipo_identificacion') == '06' ? 'selected' : '' }}>06 - Pasaporte</option>
                        <option value="07" {{ old('tipo_identificacion') == '07' ? 'selected' : '' }}>07 - Consumidor Final</option>
                        <option value="08" {{ old('tipo_identificacion') == '08' ? 'selected' : '' }}>08 - Exterior</option>
                    </select>
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="identificacion" x-model="identificacion" value="{{ old('identificacion') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="razon_social" x-model="razonSocial" value="{{ old('razon_social') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="direccion" x-model="direccion" value="{{ old('direccion') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                ...
                <div>
                    ...
                    <input type="text" name="email" value="{{ old('email') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" placeholder="email1@ejemplo.com, email2@ejemplo.com">
                    <p class="text-xs text-gray-500 mt-1">Para multiples emails, separar con coma (,)</p>
                    @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Telefono</label>
                    <input type="text" name="telefono" value="{{ old('telefono') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                    @error('telefono') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="flex justify-end mt-6">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Crear Cliente</button>
            </div>
        </form>
    </div>
    ...
    function consultaCliente() {
        return {
            tipoIdentificacion: '{{ old('tipo_identificacion', '04') }}',
