<x-emisor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Liquidacion {{ $liquidacion->establecimiento->codigo ?? '000' }}-{{ $liquidacion->ptoEmision->codigo ?? '000' }}-{{ str_pad($liquidacion->secuencial ?? 0, 9, '0', STR_PAD_LEFT) }}</h2>
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        @if(in_array($liquidacion->estado, ['CREADA', 'NO AUTORIZADO', 'DEVUELTA']))
        <a href="{{ route('emisor.comprobantes.liquidaciones.edit', $liquidacion) }}" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 text-sm">Editar</a>
        @endif
        @if(in_array($liquidacion->estado, ['CREADA', 'NO AUTORIZADO', 'RECHAZADO', 'FIRMADA', 'ENVIADA', 'PROCESANDOSE', 'EN PROCESO']))
        <form method="POST" action="{{ route('emisor.comprobantes.liquidaciones.procesar', $liquidacion) }}" onsubmit="return confirm('¿Enviar esta liquidación al SRI?')">
            @csrf
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-sm">Enviar al SRI</button>
        </form>
        @endif
        <a href="{{ route('emisor.comprobantes.liquidaciones.pdf', $liquidacion) }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 text-sm">PDF A4</a>
        <a href="{{ route('emisor.comprobantes.liquidaciones.pdf-pos', $liquidacion) }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 text-sm">PDF Ticket</a>
        @include('emisor.comprobantes.partials.email-modal', [
            'action' => route('emisor.comprobantes.liquidaciones.email', $liquidacion),
            'clienteEmail' => $liquidacion->cliente->email ?? '',
        ])
        <form method="POST" action="{{ route('emisor.comprobantes.liquidaciones.clonar', $liquidacion) }}">
            @csrf
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        <form method="POST" action="{{ route('emisor.comprobantes.liquidaciones.anular', $liquidacion) }}" onsubmit="return confirm('¿Está seguro de anular esta liquidación?\n\nEsto la marcará como ANULADA en el sistema.\nDeberá completar la anulación en el portal del SRI.')">
            @csrf
    ...
    <div class="mb-6">
        @php
            ...
            $color = $colors[$liquidacion->estado] ?? 'gray';
        @endphp
        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-{{ $color }}-100 text-{{ $color }}-800">{{ $liquidacion->estado }}</span>
        @if($liquidacion->ambiente)
        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $liquidacion->ambiente === '1' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800' }}">{{ $liquidacion->ambiente === '1' ? 'PRUEBAS' : 'PRODUCCIÓN' }}</span>
        @endif
        @if($liquidacion->motivo_rechazo)
        <div class="mt-3 bg-red-50 border-l-4 border-red-400 text-red-700 px-4 py-3 rounded-lg text-sm">
            <div class="flex items-start">
                ...
                <div>
                    <span class="font-semibold">Motivo:</span> {!! nl2br(e($liquidacion->motivo_rechazo)) !!}
                    @if(in_array($liquidacion->estado, ['NO AUTORIZADO', 'RECHAZADO', 'DEVUELTA']))
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <dl class="space-y-1 text-sm">
                <div class="flex justify-between"><dt class="text-gray-500">Razon Social:</dt><dd class="text-gray-900">{{ $liquidacion->emisor->razon_social ?? 'N/A' }}</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">RUC:</dt><dd class="text-gray-900">{{ $liquidacion->emisor->ruc ?? 'N/A' }}</dd></div>
            </dl>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-3">Proveedor</h3>
            <dl class="space-y-1 text-sm">
                <div class="flex justify-between"><dt class="text-gray-500">Razon Social:</dt><dd class="text-gray-900">{{ $liquidacion->cliente->razon_social ?? 'N/A' }}</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">Identificacion:</dt><dd class="text-gray-900">{{ $liquidacion->cliente->identificacion ?? 'N/A' }}</dd></div>
            </dl>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <dl class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div><dt class="text-gray-500">Numero:</dt><dd class="text-gray-900 font-medium">{{ $liquidacion->establecimiento->codigo ?? '000' }}-{{ $liquidacion->ptoEmision->codigo ?? '000' }}-{{ str_pad($liquidacion->secuencial ?? 0, 9, '0', STR_PAD_LEFT) }}</dd></div>
            <div><dt class="text-gray-500">Fecha Emision:</dt><dd class="text-gray-900">{{ \Carbon\Carbon::parse($liquidacion->fecha_emision)->format('d/m/Y') }}</dd></div>
            <div><dt class="text-gray-500">Clave de Acceso:</dt><dd class="text-gray-900 break-all text-xs">{{ $liquidacion->clave_acceso ?? 'N/A' }}</dd></div>
        </dl>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <p class="text-sm text-gray-900">{{ $liquidacion->observaciones }}</p>
    </div>
    @endif
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex justify-end">
            <div class="w-72 space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-gray-600">Subtotal sin impuestos:</span><span class="font-medium">${{ number_format($liquidacion->total_sin_impuestos ?? 0, 2) }}</span></div>
                <div class="flex justify-between"><span class="text-gray-600">IVA:</span><span class="font-medium">${{ number_format($liquidacion->total_iva ?? 0, 2) }}</span></div>
                <div class="flex justify-between font-bold border-t pt-2 text-base"><span>TOTAL:</span><span>${{ number_format($liquidacion->importe_total ?? 0, 2) }}</span></div>
            </div>
        </div>
    </div>

    {{-- Mensajes del SRI --}}
    @if($liquidacion->mensajes && $liquidacion->mensajes->count())
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <h3 class="text-lg font-medium text-gray-900 mb-3">Mensajes del SRI</h3>
        <div class="space-y-3">
            @foreach($liquidacion->mensajes as $mensaje)
