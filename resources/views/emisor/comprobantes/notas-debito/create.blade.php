<x-emisor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Nueva Nota de Debito</h2>
    ...
    <form method="POST" action="{{ route('emisor.comprobantes.notas-debito.store') }}" id="nd-form">
        @csrf
        ...
        <div class="flex justify-end">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Guardar Nota de Debito</button>
        </div>
    </form>
    ...
    <script>
        ...
        function buildIvaOptions() {
            return ivasData.map(iva =>
                `<option value="${iva.id}" data-tarifa="${iva.tarifa}">${iva.nombre} (${iva.tarifa}%)</option>`
            ).join('');
        }
        ...
        function addMotivo() {
            ...
            tr.innerHTML = `
                <td class="px-3 py-2"><input type="text" name="motivos[${motivoIndex}][razon]" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required></td>
                <td class="px-3 py-2"><input type="number" name="motivos[${motivoIndex}][valor]" class="w-full border-gray-300 rounded-md shadow-sm text-sm valor" value="0.00" min="0" step="any" required></td>
                <td class="px-3 py-2">
        ...
        document.getElementById('agregar-motivo').addEventListener('click', addMotivo);
