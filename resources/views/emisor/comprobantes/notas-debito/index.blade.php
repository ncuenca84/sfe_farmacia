<x-emisor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Notas de Debito</h2>
            <a href="{{ route('emisor.comprobantes.notas-debito.create') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white px-5 py-2.5 rounded-xl hover:from-blue-700 hover:to-blue-800 text-sm font-medium shadow-sm shadow-blue-500/25 transition-all duration-150">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-6">
        <form method="GET" action="{{ route('emisor.comprobantes.notas-debito.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
                ...
                <select name="estado" class="w-full border-gray-200 rounded-lg shadow-sm text-sm focus:border-blue-500 focus:ring-blue-500">
                    ...
                    @foreach(['CREADA','FIRMADA','ENVIADA','AUTORIZADO','RECHAZADO','ANULADA'] as $est)
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table class="min-w-full">
            <thead>
                <tr class="bg-gray-50/80 border-b border-gray-100">
                    ...
                    <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Doc Modificado</th>
                    <th class="px-6 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Valor Total</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-blue-50/30 transition-colors duration-100">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {{ $nd->establecimiento->codigo ?? '000' }}-{{ $nd->ptoEmision->codigo ?? '000' }}-{{ str_pad($nd->secuencial ?? 0, 9, '0', STR_PAD_LEFT) }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ \Carbon\Carbon::parse($nd->fecha_emision)->format('d/m/Y') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $nd->cliente->razon_social ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $nd->num_doc_modificado ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-right">${{ number_format($nd->importe_total ?? 0, 2) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @php
                            $colors = ['CREADA' => 'gray', 'AUTORIZADO' => 'green', 'RECHAZADO' => 'red', 'ANULADA' => 'yellow', 'ENVIADA' => 'blue', 'FIRMADA' => 'blue'];
                            $color = $colors[$nd->estado] ?? 'gray';
                        @endphp
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="{{ route('emisor.comprobantes.notas-debito.show', $nd) }}" class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-700 text-xs font-semibold rounded-lg hover:bg-blue-100 ring-1 ring-blue-600/10 transition-colors duration-150">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <svg class="mx-auto h-10 w-10 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/></svg>
                        <p class="mt-2 text-sm text-gray-500">No se encontraron notas de debito.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
            {{ $notasDebito->withQueryString()->links() }}
        </div>
    </div>
</x-emisor-layout>
