<x-emisor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Nueva Guia de Remision</h2>
    ...
    <form method="POST" action="{{ route('emisor.comprobantes.guias.store') }}" id="guia-form">
        @csrf
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="dir_partida" value="{{ old('dir_partida', ($prefill ?? [])['dir_partida'] ?? '') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required placeholder="Direccion de origen">
                ...
                <div>
                    ...
                    <input type="text" name="dir_llegada" value="{{ old('dir_llegada', ($prefill ?? [])['dir_llegada'] ?? '') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" placeholder="Direccion de destino">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <x-transportista-search
                    :initial-ruc="($prefill ?? [])['ruc_transportista'] ?? ''"
                    :initial-razon-social="($prefill ?? [])['razon_social_transportista'] ?? ''"
                    :initial-placa="($prefill ?? [])['placa'] ?? ''"
                />
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Fecha Inicio Transporte *</label>
                    <input type="date" name="fecha_inicio_transporte" value="{{ old('fecha_inicio_transporte', ($prefill ?? [])['fecha_inicio_transporte'] ?? '') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                ...
                <div>
                    ...
                    <input type="date" name="fecha_fin_transporte" value="{{ old('fecha_fin_transporte', ($prefill ?? [])['fecha_fin_transporte'] ?? '') }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
        ...
        <div class="flex justify-end">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Guardar Guia de Remision</button>
        </div>
    </form>
    ...
    <script>
        ...
        function guiaProdSelect(producto) {
            window.dispatchEvent(new CustomEvent('guia-product-selected', { detail: { producto: producto, idx: _guiaProdTargetIdx } }));
        ...
        var _guiaPrefillDestinatarios = @json(($prefill ?? [])['destinatarios'] ?? []);

        function destinatariosManager() {
            return {
                destinatarios: _guiaPrefillDestinatarios.length > 0
                    ? _guiaPrefillDestinatarios.map(function(d) {
                        var dest = newDestinatario();
                        dest.identificacion = d.identificacion || '';
                        dest.razon_social = d.razon_social || '';
                        dest.direccion = d.direccion || '';
                        dest.motivo_traslado = d.motivo_traslado || '';
                        if (d.identificacion && d.razon_social) {
                            dest.selectedDisplay = d.identificacion + ' - ' + d.razon_social;
                        }
                        if (d.productos && d.productos.length > 0) {
                            dest.productos = d.productos.map(function(p) {
                                return { codigo: p.codigo_principal || '', descripcion: p.descripcion || '', cantidad: p.cantidad || 1 };
                            });
                        }
                        return dest;
                    })
