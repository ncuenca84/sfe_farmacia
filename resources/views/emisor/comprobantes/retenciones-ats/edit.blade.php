<x-emisor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Editar Retencion ATS #{{ $retencionAt->id }}</h2>
            <a href="{{ route('emisor.comprobantes.retenciones-ats.show', $retencionAt) }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 text-sm">Volver</a>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('emisor.comprobantes.retenciones-ats.update', $retencionAt) }}" id="retencion-ats-form">
        @csrf
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="date" name="fecha_emision" value="{{ old('fecha_emision', $retencionAt->fecha_emision?->format('Y-m-d') ?? date('Y-m-d')) }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Periodo Fiscal (MM/YYYY)</label>
                    <input type="text" name="periodo_fiscal" value="{{ old('periodo_fiscal', $retencionAt->periodo_fiscal) }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required maxlength="7">
                </div>
                <div>
                    ...
                    <select name="parte_rel" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                        <option value="NO" {{ old('parte_rel', $retencionAt->parte_rel ?? 'NO') == 'NO' ? 'selected' : '' }}>NO</option>
                        <option value="SI" {{ old('parte_rel', $retencionAt->parte_rel) == 'SI' ? 'selected' : '' }}>SI</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <x-cliente-search :selectedId="$retencionAt->cliente_id" :selectedDisplay="($retencionAt->cliente->identificacion ?? '') . ' - ' . ($retencionAt->cliente->razon_social ?? '')" />
            </div>
        </div>
        ...
        <div class="flex justify-between mb-6">
            ...
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Actualizar Retencion ATS</button>
        </div>
    </form>
    ...
    <script>
        ...
        @php
            $existingData = $retencionAt->docSustentos->map(function($ds) {
                return [
                    'cod_sustento' => $ds->cod_sustento,
                    'cod_doc_sustento' => $ds->cod_doc_sustento,
                    'num_doc_sustento' => $ds->num_doc_sustento,
                    'fecha_emision_doc_sustento' => $ds->fecha_emision_doc_sustento->format('Y-m-d'),
                    'fecha_registro_contable' => $ds->fecha_registro_contable?->format('Y-m-d'),
                    'num_aut_doc_sustento' => $ds->num_aut_doc_sustento,
                    'pago_loc_ext' => $ds->pago_loc_ext ?? '01',
                    'total_sin_impuestos' => $ds->total_sin_impuestos,
                    'importe_total' => $ds->importe_total,
                    'forma_pago' => $ds->forma_pago ?? '20',
                    'impuestos' => $ds->impuestos->map(fn($imp) => [
                    ...
                    'desgloses' => $ds->desgloses->map(fn($d) => [
                        'codigo_impuesto' => $d->codigo_impuesto,
        ...
        @endphp
        const existingDocSustentos = @json($existingData);
        ...
        function addDocSustento(data) {
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <select name="doc_sustentos[${dsIdx}][cod_sustento]" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                            ${buildOptions(COD_SUSTENTOS, data.cod_sustento || '01')}
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="doc_sustentos[${dsIdx}][cod_doc_sustento]" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                            ${buildOptions(COD_DOC_SUSTENTOS, data.cod_doc_sustento || '01')}
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Num Doc Sustento</label>
                        <input type="text" name="doc_sustentos[${dsIdx}][num_doc_sustento]" value="${data.num_doc_sustento || '001001000000001'}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Fecha Emision Doc Sustento</label>
                        <input type="date" name="doc_sustentos[${dsIdx}][fecha_emision_doc_sustento]" value="${data.fecha_emision_doc_sustento || ''}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Fecha Registro Contable</label>
                        <input type="date" name="doc_sustentos[${dsIdx}][fecha_registro_contable]" value="${data.fecha_registro_contable || ''}" class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Num Aut Doc Sustento</label>
                        <input type="text" name="doc_sustentos[${dsIdx}][num_aut_doc_sustento]" value="${data.num_aut_doc_sustento || ''}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" maxlength="49">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <div>
                        ...
                        <select name="doc_sustentos[${dsIdx}][pago_loc_ext]" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                            ${buildOptions(PAGO_LOC_EXT, data.pago_loc_ext || '01')}
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Total Sin Impuestos *</label>
                        <input type="number" name="doc_sustentos[${dsIdx}][total_sin_impuestos]" value="${data.total_sin_impuestos || '0.00'}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" min="0" step="any" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Importe Total *</label>
                        <input type="number" name="doc_sustentos[${dsIdx}][importe_total]" value="${data.importe_total || '0.00'}" class="w-full border-gray-300 rounded-md shadow-sm text-sm" min="0" step="any" required>
                    </div>
                    <div>
                        ...
                        <select name="doc_sustentos[${dsIdx}][forma_pago]" class="w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                            ${buildOptions(FORMAS_PAGO, data.forma_pago || '20')}
                        </select>
                    </div>
                </div>
            ...
            div.querySelector('.remove-doc-sustento').addEventListener('click', function() { div.remove(); });
            div.querySelector('.agregar-impuesto-doc').addEventListener('click', function() { addImpuestoDoc(dsIdx, div); });
            div.querySelector('.agregar-retencion').addEventListener('click', function() { addRetencion(dsIdx, div); });

            // Load existing impuestos doc
            if (data.impuestos && data.impuestos.length > 0) {
                data.impuestos.forEach(imp => addImpuestoDoc(dsIdx, div, imp));
            } else {
                addImpuestoDoc(dsIdx, div);
            }

            // Load existing retenciones
            if (data.desgloses && data.desgloses.length > 0) {
                data.desgloses.forEach(d => addRetencion(dsIdx, div, d));
            } else {
        ...
        let impDocCounters = {};
        function addImpuestoDoc(dsIdx, dsDiv, data) {
            data = data || {};
            ...
            tr.innerHTML = `
                <td class="px-3 py-2">
                    <select name="doc_sustentos[${dsIdx}][impuestos_doc][${idx}][codigo_impuesto]" class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                        <option value="2" ${(data.codigo_impuesto || '2') == '2' ? 'selected' : ''}>IVA</option>
                        <option value="3" ${data.codigo_impuesto == '3' ? 'selected' : ''}>ICE</option>
                        <option value="5" ${data.codigo_impuesto == '5' ? 'selected' : ''}>IRBPNR</option>
                    </select>
                </td>
                <td class="px-3 py-2">
                    <select name="doc_sustentos[${dsIdx}][impuestos_doc][${idx}][codigo_porcentaje]" class="w-full border-gray-300 rounded-md shadow-sm text-sm iva-porcentaje-select">
                        ${IVA_PORCENTAJES.map(p => `<option value="${p.code}" data-tarifa="${p.tarifa}" ${p.code == (data.codigo_porcentaje || '0') ? 'selected' : ''}>${p.desc}</option>`).join('')}
                    </select>
                </td>
                <td class="px-3 py-2"><input type="number" name="doc_sustentos[${dsIdx}][impuestos_doc][${idx}][base_imponible]" class="w-full border-gray-300 rounded-md shadow-sm text-sm imp-base" value="${data.base_imponible || '0.00'}" min="0" step="any" required></td>
                <td class="px-3 py-2"><input type="number" name="doc_sustentos[${dsIdx}][impuestos_doc][${idx}][tarifa]" class="w-full border-gray-300 rounded-md shadow-sm text-sm imp-tarifa" value="${data.tarifa || '0'}" min="0" step="any" required></td>
                <td class="px-3 py-2"><input type="number" name="doc_sustentos[${dsIdx}][impuestos_doc][${idx}][valor_impuesto]" class="w-full border-gray-300 rounded-md shadow-sm text-sm imp-valor" value="${data.valor_impuesto || '0.00'}" min="0" step="any" required></td>
        ...
        let retCounters = {};
        function addRetencion(dsIdx, dsDiv, data) {
            data = data || {};
            ...
            tr.innerHTML = `
                <td class="px-3 py-2">
                    <select name="doc_sustentos[${dsIdx}][retenciones][${idx}][codigo]" class="w-full border-gray-300 rounded-md shadow-sm text-sm impuesto-tipo">
                        <option value="1" ${(data.codigo_impuesto || '1') == '1' ? 'selected' : ''}>Renta</option>
                        <option value="2" ${data.codigo_impuesto == '2' ? 'selected' : ''}>IVA</option>
                        <option value="6" ${data.codigo_impuesto == '6' ? 'selected' : ''}>ISD</option>
                    </select>
                </td>
                <td class="px-3 py-2">
                    <div class="flex gap-1">
                        <input type="text" name="doc_sustentos[${dsIdx}][retenciones][${idx}][codigo_retencion]" value="${data.codigo_retencion || ''}" class="w-full border-gray-300 rounded-md shadow-sm text-sm codigo-retencion-input" required>
                ...
                <td class="px-3 py-2"><input type="number" name="doc_sustentos[${dsIdx}][retenciones][${idx}][porcentaje_retener]" class="w-full border-gray-300 rounded-md shadow-sm text-sm porcentaje" value="${data.porcentaje_retener || '0'}" min="0" step="any" required></td>
                <td class="px-3 py-2"><input type="number" name="doc_sustentos[${dsIdx}][retenciones][${idx}][base_imponible]" class="w-full border-gray-300 rounded-md shadow-sm text-sm base" value="${data.base_imponible || '0.00'}" min="0" step="any" required></td>
                <td class="px-3 py-2 text-right">
                    <span class="valor-text">$${((parseFloat(data.base_imponible) || 0) * (parseFloat(data.porcentaje_retener) || 0) / 100).toFixed(2)}</span>
                    <input type="hidden" name="doc_sustentos[${dsIdx}][retenciones][${idx}][valor_retenido]" class="valor-retenido-hidden" value="${data.valor_retenido || '0.00'}">
                </td>
        ...
        function loadCodigos() {
            ...
            fetch(`{{ route('emisor.codigos-retencion.buscar') }}?${params.toString()}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('modal-info').textContent = `Mostrando del ${data.from} al ${data.to} de ${data.total} registros`;
        ...
        document.getElementById('agregar-doc-sustento').addEventListener('click', function() { addDocSustento(); });
        ...
        // Load existing doc sustentos
        if (existingDocSustentos.length > 0) {
            existingDocSustentos.forEach(ds => addDocSustento(ds));
        } else {
            addDocSustento();
        }
    </script>
