...
<div class="ticket">
    ...
    {{-- ============== DOCUMENTO ============== --}}
    <div class="doc-title">RETENCION</div>
    ...
    <div class="field-value center">IMPUESTOS RETENIDOS</div>
    <div class="separator"></div>

    @foreach($ret->impuestosRetencion as $imp)
    @php $totalRetenido += $imp->valor_retenido; @endphp
    <div class="ret-block">
        <div class="field-value">{{ $codImpuestos[$imp->codigo_impuesto] ?? $imp->codigo_impuesto }} - Cod: {{ $imp->codigo_retencion }}</div>
        <div class="field-label">Base: {{ number_format($imp->base_imponible, 2) }} | %: {{ number_format($imp->porcentaje_retener, 2) }}%</div>
        <div class="field-value">Valor Ret: {{ number_format($imp->valor_retenido, 2) }}</div>
        <div class="field-label">Doc: {{ $imp->cod_doc_sustento }} {{ $imp->num_doc_sustento }}</div>
        @if($imp->fecha_emision_doc_sustento)
        <div class="field-label">F. Doc: {{ $imp->fecha_emision_doc_sustento->format('d/m/Y') }}</div>
        @endif
    </div>
    @endforeach
