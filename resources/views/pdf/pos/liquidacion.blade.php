...
<div class="ticket">
    ...
    <div class="logo-area">
        @if($logoPath)
            <img src="{{ $logoPath }}">
        @else
            <div class="company-name">{{ $liq->emisor->nombre_comercial ?? $liq->emisor->razon_social }}</div>
        @endif
    </div>
    <div class="company-name">{{ $liq->emisor->razon_social }}</div>
    <div class="info-line">RUC: {{ $liq->emisor->ruc }}</div>
    <div class="info-line">Dir: {{ $liq->emisor->direccion_matriz }}</div>
    @if($liq->establecimiento->direccion ?? false)
    <div class="info-line">Suc: {{ $liq->establecimiento->direccion }}</div>
    @endif
    @if($liq->emisor->contribuyente_especial)
    <div class="info-line">Contrib. Especial: {{ $liq->emisor->contribuyente_especial }}</div>
    @endif
    <div class="info-line">Obligado Contab: {{ $liq->emisor->obligado_contabilidad ? 'SI' : 'NO' }}</div>
    @if($liq->emisor->agente_retencion)
    <div class="info-line">Agente Ret. Nro: {{ $liq->emisor->agente_retencion }}</div>
    @endif
    @if($liq->emisor->regimen?->esRimpe())
    <div class="info-line">{{ $liq->emisor->regimen->nombre() }}</div>
    @endif

    <div class="separator-double"></div>

    {{-- ============== DOCUMENTO ============== --}}
    <div class="doc-title">LIQ. DE COMPRA</div>
    <div class="doc-number">{{ $liq->establecimiento->codigo }}-{{ $liq->ptoEmision->codigo }}-{{ str_pad($liq->secuencial, 9, '0', STR_PAD_LEFT) }}</div>
    <div class="info-line">Fecha: {{ $liq->fecha_emision->format('d/m/Y') }}</div>
    <div class="info-line">Ambiente: {{ ($liq->ambiente ?? $liq->emisor->ambiente->value) == '1' ? 'PRUEBAS' : 'PRODUCCION' }}</div>

    <div class="separator"></div>

    {{-- ============== PROVEEDOR ============== --}}
    <div class="field-value">PROVEEDOR: {{ $liq->cliente->razon_social }}</div>
    <div class="field-label">RUC/CI: {{ $liq->cliente->identificacion }}</div>
    @if($liq->cliente->direccion)
    <div class="field-label">Dir: {{ $liq->cliente->direccion }}</div>
    @endif
    ...
    <table class="totals">
        ...
        @endif
        <tr><td class="bold">SUBTOTAL</td><td class="r bold">{{ number_format($liq->total_sin_impuestos, 2) }}</td></tr>
        @if($liq->total_descuento > 0)
        <tr><td>DESCUENTO</td><td class="r">{{ number_format($liq->total_descuento, 2) }}</td></tr>
        @endif
        @if($iva15 > 0)
        ...
        @endif
        <tr class="total-row"><td>TOTAL</td><td class="r">{{ number_format($liq->importe_total, 2) }}</td></tr>
    </table>

    <div class="separator"></div>

    {{-- ============== FORMA DE PAGO ============== --}}
    <div class="field-label">F. PAGO: {{ $formas[$liq->forma_pago] ?? $liq->forma_pago }}</div>
    <div class="field-value" style="text-align: right;">{{ number_format($liq->forma_pago_valor, 2) }}</div>

    {{-- ============== INFO ADICIONAL ============== --}}
    @if($liq->observaciones)
    <div class="separator"></div>
    <div class="field-label">OBS: {{ $liq->observaciones }}</div>
    @endif

    @if($liq->camposAdicionales && $liq->camposAdicionales->count())
    <div class="separator"></div>
    ...
    <div class="center">
        ...
        @else
            <span class="badge">{{ $liq->estado }}</span>
        @endif
    </div>

    @if($liq->numero_autorizacion ?? $liq->clave_acceso)
    <div class="info-line" style="margin-top: 2px;">Aut: {{ $liq->numero_autorizacion ?? $liq->clave_acceso ?? 'PENDIENTE' }}</div>
    @endif
    @if($liq->fecha_autorizacion)
    <div class="info-line">F. Aut: {{ $liq->fecha_autorizacion->format('d/m/Y H:i:s') }}</div>
    @endif
    ...
    @if($liq->clave_acceso)
    <div class="clave-text">{{ $liq->clave_acceso }}</div>
    @endif
