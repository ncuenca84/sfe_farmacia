...
<div class="page-wrapper">
                ...
                <div class="logo-emisor-box">
                    <div class="logo-area">
                        @if($logoPath)
                            <img src="{{ $logoPath }}">
                        @else
                            <div class="logo-fallback">{{ $liq->emisor->nombre_comercial ?? $liq->emisor->razon_social }}</div>
                        @endif
                    </div>
                    <div class="emisor-name">{{ $liq->emisor->razon_social }}</div>
                    <div class="emisor-detail"><b>DIR MATRIZ:</b> {{ $liq->emisor->direccion_matriz }}</div>
                    <div class="emisor-detail"><b>DIR SUCURSAL:</b> {{ $liq->establecimiento->direccion ?? '' }}</div>
                    <div class="emisor-detail"><b>CONTRIBUYENTE ESPECIAL NRO:</b> {{ $liq->emisor->contribuyente_especial ?? '' }}</div>
                    <div class="emisor-detail"><b>OBLIGADO A LLEVAR CONTABILIDAD:</b> {{ $liq->emisor->obligado_contabilidad ? 'SI' : 'NO' }}</div>
                    @if($liq->emisor->agente_retencion)
                        <div class="emisor-detail"><b>AGENTE DE RETENCIÓN RES. NRO:</b> {{ $liq->emisor->agente_retencion }}</div>
                    @endif
                    @if($liq->emisor->regimen?->esRimpe())
                        <div class="emisor-badge">{{ $liq->emisor->regimen->nombre() }}</div>
                    @endif
                </div>
            </td>
            <td style="width: 48%;">
                <div class="doc-box">
                    <div class="doc-header">
                        <div class="doc-ruc">R.U.C.: {{ $liq->emisor->ruc }}</div>
                        <div class="doc-type">LIQUIDACIÓN DE COMPRA</div>
                        <div class="doc-number">No. {{ $liq->establecimiento->codigo }}-{{ $liq->ptoEmision->codigo }}-{{ str_pad($liq->secuencial, 9, '0', STR_PAD_LEFT) }}</div>
                    </div>
                    <div class="doc-body">
                        <div class="doc-field">
                            <div class="doc-field-label">Número de Autorización</div>
                            <div class="doc-field-value">{{ $liq->numero_autorizacion ?? $liq->clave_acceso ?? 'PENDIENTE' }}</div>
                        </div>
                        <div class="doc-field">
                            <div class="doc-field-label">Fecha y Hora de Autorización</div>
                            <div class="doc-field-value">{{ $liq->fecha_autorizacion ? $liq->fecha_autorizacion->format('d/m/Y H:i:s') : 'PENDIENTE' }}</div>
                        </div>
                        <table class="layout" style="margin-top: 4px;">
                            <tr>
                                <td style="width: 50%; text-align: center; padding: 3px; background: #f7fafc; border: 1px solid #e2e8f0; font-size: 8px;">
                                    ...
                                    {{ ($liq->ambiente ?? $liq->emisor->ambiente->value) == '1' ? 'PRUEBAS' : 'PRODUCCIÓN' }}
                                </td>
                                <td style="width: 50%; text-align: center; padding: 3px; background: #f7fafc; border: 1px solid #e2e8f0; font-size: 8px;">
                ...
                <div class="clave-box">
                    ...
                    @endif
                    <div class="clave-number">{{ $liq->clave_acceso }}</div>
                </div>
                @endif
            </td>
        </tr>
    ...
    <table class="cliente-table">
        <tr><td colspan="4" class="section-header">DATOS DEL PROVEEDOR</td></tr>
        <tr>
            <td class="lbl">Razón Social / Nombres:</td>
            <td colspan="3">{{ $liq->cliente->razon_social }}</td>
        </tr>
        <tr>
            <td class="lbl">Identificación:</td>
            <td>{{ $liq->cliente->identificacion }}</td>
            <td class="lbl" style="width: 110px;">Fecha de Emisión:</td>
            <td>{{ $liq->fecha_emision->format('d/m/Y') }}</td>
        </tr>
    </table>
        ...
        <tr>
            <td style="width: 55%; padding-right: 6px;">
                ...
                <table style="width: 100%; border-collapse: collapse; border: 1px solid #cbd5e0; margin-bottom: 5px;">
                    ...
                    <tr><td style="padding: 4px 6px; font-size: 7.5px;">{{ $liq->observaciones }}</td></tr>
                </table>
                @endif

                @if(($liq->camposAdicionales && $liq->camposAdicionales->count()) || $liq->cliente->direccion || $liq->cliente->telefono || $liq->cliente->email)
                <table style="width: 100%; border-collapse: collapse; border: 1px solid #cbd5e0; margin-bottom: 5px;">
                    <tr><td colspan="2" class="section-header">Información Adicional</td></tr>
                    @if($liq->cliente->direccion)
                    <tr><td style="padding: 1px 6px; font-size: 7.5px; width: 70px; font-weight: bold; color: #4a5568;">Dirección:</td><td style="padding: 1px 6px; font-size: 7.5px;">{{ $liq->cliente->direccion }}</td></tr>
                    @endif
                    @if($liq->cliente->telefono)
                    <tr><td style="padding: 1px 6px; font-size: 7.5px; width: 70px; font-weight: bold; color: #4a5568;">Teléfono:</td><td style="padding: 1px 6px; font-size: 7.5px;">{{ $liq->cliente->telefono }}</td></tr>
                    @endif
                    @if($liq->cliente->email)
                    <tr><td style="padding: 1px 6px; font-size: 7.5px; width: 70px; font-weight: bold; color: #4a5568;">Email:</td><td style="padding: 1px 6px; font-size: 7.5px;">{{ $liq->cliente->email }}</td></tr>
                    @endif
                    @if($liq->camposAdicionales)
                        @foreach($liq->camposAdicionales as $campo)
                ...
                <table style="width: 100%; border-collapse: collapse; border: 1px solid #cbd5e0;">
                    ...
                    <tr>
                        <td style="padding: 3px 6px; font-size: 7.5px;">{{ $formas[$liq->forma_pago] ?? $liq->forma_pago }}</td>
                        <td style="padding: 3px 6px; font-size: 7.5px; text-align: right; font-weight: bold;">{{ number_format($liq->forma_pago_valor, 2) }}</td>
                    </tr>
                </table>
            </td>
            <td style="width: 45%;">
                <table class="totales-table">
                    ...
                    <tr style="border-bottom: 2px solid #cbd5e0;"><td class="lbl">SUBTOTAL SIN IMPUESTOS</td><td class="val" style="font-weight: bold;">{{ number_format($liq->total_sin_impuestos, 2) }}</td></tr>
                    <tr><td class="lbl">TOTAL Descuento</td><td class="val">{{ number_format($liq->total_descuento, 2) }}</td></tr>
                    @if($iva15 > 0)
                    ...
                    @endif
                    <tr class="total-row"><td>IMPORTE TOTAL</td><td style="text-align: right;">{{ number_format($liq->importe_total, 2) }}</td></tr>
                </table>
            </td>
        </tr>
    </table>
    ...
    <table class="footer-table">
        <tr>
            ...
            <td>
                ...
                @endif
                <div class="footer-clave">{{ $liq->clave_acceso }}</div>
            </td>
            <td style="width: 95px;">
                <div class="footer-label">Estado</div>
                @if($liq->estado === 'AUTORIZADO')
                    <span class="auth-badge auth-autorizado">AUTORIZADO</span>
                @elseif($liq->estado === 'NO AUTORIZADO')
                    <span class="auth-badge auth-no-autorizado">NO AUTORIZADO</span>
                @else
                    <span class="auth-badge auth-pendiente">{{ $liq->estado }}</span>
                @endif
                @if($liq->fecha_autorizacion)
                    <div style="font-size: 7px; color: #718096; margin-top: 3px;">
                        {{ $liq->fecha_autorizacion->format('d/m/Y') }}<br>
                        {{ $liq->fecha_autorizacion->format('H:i:s') }}
                    </div>
                @endif
            </td>
        </tr>
